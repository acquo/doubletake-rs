...
//! `~/.config/doubletake/credentials.json` files keep working:
//!
//! ```json
//! {
//!   "<deviceID>": {
...
impl CredentialStore {
    ...
    /// Returns the default path `~/.config/doubletake/credentials.json`.
    pub fn default_path() -> PathBuf {
        let home = std::env::var("USERPROFILE")
            .or_else(|_| std::env::var("HOME"))
            .unwrap_or_else(|_| ".".into());
        Path::new(&home).join(".config").join("doubletake").join("credentials.json")
    }
