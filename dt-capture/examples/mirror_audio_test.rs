...
//! and the RTP/sync packet flow against `doubletake-test-receiver`).
//!
//! Usage: mirror_audio_test <host> <port> <pin> [frames]
//! Receiver: doubletake-test-receiver.exe -auth pin -code 1234 -profile roku
//!           -listen 127.0.0.1:7100 -debug -stats-interval 2s
//! Check the receiver stats line for `audio=N/B`.

use dt_airplay::audio::{encode_alac_verbatim, SPF};
use dt_airplay::client::Client;
use dt_airplay::mirror::{ntp_network_time, setup_mirror_session};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if info.supports_fairplay_sap() {
        let fp = dt_airplay::fairplay::fairplay_setup(&mut client, &info, &pairing.keys.shared_secret, true)?;
