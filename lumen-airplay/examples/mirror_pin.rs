//! Interop test: full mirror session against the Go `lumen-test-receiver`.
//!
//! Usage:
...
use lumen_airplay::h264::{build_avcc_config, nal_type, strip_start_code, sps_dimensions, H264Parser};
        ...
        for nal in nals {
            ...
            match nt {
                ...
                5 => {
                    ...
                    vcl_buf.extend_from_slice(&lumen_airplay::h264::avcc_wrap(&raw));
                }
                1..=4 => {
                    if !vcl_buf.is_empty() && pending_keyframe {
                        flush_vcl(session, &mut vcl_buf, &latest_sps, &latest_pps, &mut pending_keyframe, &mut codec_sent)?;
                    ...
                    if !vcl_buf.is_empty() && !pending_keyframe && lumen_airplay::h264::is_first_slice(&raw) {
                        flush_vcl(session, &mut vcl_buf, &latest_sps, &latest_pps, &mut pending_keyframe, &mut codec_sent)?;
                    }
                    vcl_buf.extend_from_slice(&lumen_airplay::h264::avcc_wrap(&raw));
                }
